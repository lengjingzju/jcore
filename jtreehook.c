...
#define JHOOK_FUNC_LEN      128
    char func[JHOOK_DEPTH][JHOOK_FUNC_LEN]; // 记录函数名称
    ...
    if (!node) {
        ...
        while (unw_step(&cursor) > 0) {
            if (i >= JHOOK_DEPTH) {
                ...
                node->func[i][0] = '\0';
                node->offset[i] = 0;
                ret = unw_get_proc_name(&cursor, &node->func[i][0], JHOOK_FUNC_LEN, &node->offset[i]);
            }
            ++i;
        }
        for ( ; i < JHOOK_DEPTH; ++i) {
...
void jhook_addptr(void *ptr, size_t size, void *addr)
{
    ...
    key.size = size;
    memcpy(&key.addrs[0], &addrs[JHOOK_DJUMP], JHOOK_DEPTH * sizeof(void *));
    ...
    if (t) {
        ...
        node->size = size;
        memcpy(&node->addrs[0], &addrs[JHOOK_DJUMP], JHOOK_DEPTH * sizeof(void *));
        node->alloc = 1;
        node->free = 0;
        node->changed = 1;
